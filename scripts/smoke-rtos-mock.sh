...
usage() {
  cat <<'EOF'
Usage:
  scripts/smoke-rtos-mock.sh [options]

Options:
  --output-dir PATH         Artifact root. Default: artifacts/live-smoke/YYYYMMDD/rtos-mock
  --wav PATH                Optional primary WAV. If omitted, a silence WAV is generated.
  --interrupt-wav PATH      Optional interrupt WAV. Default: same as primary input.
  --desktop-python CMD      Python executable for the RTOS mock. Default: python3
  ...
  --server-port PORT        agentd bind port. Default: 18080
  --worker-host HOST        FunASR worker bind host. Default: 127.0.0.1
  --worker-port PORT        FunASR worker bind port. Default: 18091
  ...
  --timeout-sec SEC         RTOS mock timeout. Default: 60
  --llm-provider NAME       agentd LLM provider. Default: bootstrap
  --tts-provider NAME       agentd TTS provider. Default: none
  --enable-barge-in         Send an interrupt turn using the interrupt WAV.
  --help                    Show this message
EOF
}

log() {
  printf '[smoke-rtos-mock] %s\n' "$*"
}
...
if [[ -z "$OUTPUT_DIR" ]]; then
  OUTPUT_DIR="${REPO_ROOT}/artifacts/live-smoke/$(date +%Y%m%d)/rtos-mock"
fi

mkdir -p "$OUTPUT_DIR"

INPUT_WAV="${OUTPUT_DIR}/input.wav"
INTERRUPT_WAV="${OUTPUT_DIR}/interrupt-input.wav"
REPORT_PATH="${OUTPUT_DIR}/report.json"
RX_PATH="${OUTPUT_DIR}/received-audio.wav"
...
if [[ -n "$INTERRUPT_WAV_INPUT" ]]; then
  cp "$INTERRUPT_WAV_INPUT" "$INTERRUPT_WAV"
elif [[ "$ENABLE_BARGE_IN" -eq 1 ]]; then
  cp "$INPUT_WAV" "$INTERRUPT_WAV"
fi
...
if [[ "$ENABLE_BARGE_IN" -eq 1 ]]; then
  args+=(--interrupt-wav "$INTERRUPT_WAV")
fi
