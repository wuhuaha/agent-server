...
try {
  ...
  try {
    $env:PYTHONPATH = "src"
    python -m agent_server_desktop_client.runner --scenario full --http-base "http://127.0.0.1:$ServerPort" --wav $wavPath --timeout-sec 30 --output $reportPath --save-rx-dir $OutputDir
  } finally {
    Pop-Location
  }
}
